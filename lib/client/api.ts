/**
 * Browser API client. Talks to this app's own Route Handlers under `/api`
 * (same-origin), so the session cookie rides along automatically; the bearer
 * token from localStorage is still sent as a belt-and-braces fallback and to
 * support the Google OAuth hash-token hand-off.
 */
const BASE_URL = "/api";
const TOKEN_KEY = "yeep_token";

/** Base path of the API — used for full-page redirects (OAuth). */
