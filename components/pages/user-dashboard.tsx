...
export default function UserDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-100/70 flex">
      {/* Sidebar */}
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-[#0b1a19] border-r border-white/[0.06] flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        {/* Logo */}
        <Link
          href="/"
            ...
            return (
              ...
              >
                ...
                {item.id === "notifications" && unread > 0 && (
                  <span
                    className={`ml-auto min-w-5 h-5 px-1 rounded-full text-[11px] font-semibold flex items-center justify-center ${isActive ? "bg-white/20 text-white" : "bg-red-500 text-white"}`}
                  >
                    {unread}
                  </span>
                )}
            ...
            <>
                  ...
                  <div className="bg-white rounded-2xl border border-slate-200/70 shadow-sm p-6">
                    <div className="flex items-center justify-between mb-5">
                      ...
                      <button
                        ...
                        className="text-xs text-[#0f766e] font-medium flex items-center gap-1"
                      >
                    ...
                    <div className="space-y-3">
                      {data.applications.slice(0, 3).map((app) => (
                        <div
                          key={app._id}
                          className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-50/70 transition-colors"
                        >
                          ...
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full ${statusPill(app.status)}`}
                          >
                            {app.status}
                          </span>
                        </div>
                      ...
                      {data.applications.length === 0 && (
                        <p className="text-sm text-gray-400 py-4 text-center">
                          No applications yet.{" "}
                          <Link href="/volunteer" className="text-[#0f766e] font-medium">
                            Apply to volunteer →
                          </Link>
                        </p>
                      )}
                    ...
                    <div className="space-y-3">
                      {data.upcomingEvents.map((ev) => (
                        ...
                        >
                          ...
                          <Link
                            href="/events"
                            className="ml-auto text-xs text-[#0f766e] font-medium border border-[#0f766e] px-3 py-1 rounded-lg hover:bg-teal-50 transition-colors"
                          >
                            Register
                          </Link>
                        </div>
                    ...
                    {data.applications.map((app) => (
                      <div key={app._id} className="p-6 flex items-center gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-400">
                            Applied {formatDateShort(app.createdAt)}
                          </div>
                        </div>
                        <span
                          className={`text-xs px-2.5 py-1 rounded-full font-medium ${statusPill(app.status)}`}
                        >
                          {app.status}
                        </span>
                      </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-200/70 shadow-sm overflow-hidden">
                  ...
                  <div className="divide-y divide-slate-100">
                    {notifications.map((n, i) => (
                      <div
                        key={i}
                        className={`p-5 flex items-start gap-4 ${!n.read ? "bg-teal-50/30" : ""}`}
                      >
                        <div
                          className={`w-2 h-2 rounded-full mt-2 shrink-0 ${n.read ? "bg-gray-200" : "bg-[#0f766e]"}`}
                        />
                        <div className="flex-1">
              ...
              {active === "profile" && (
                <form
                  ...
                  className="bg-white rounded-2xl border border-slate-200/70 shadow-sm p-8 max-w-xl"
                >
                  <input
                    ref={fileInputRef}
                    type="file"
              ...
              {active === "settings" && (
                <div className="max-w-xl space-y-6">
                  ...
                  <form
                    ...
                    className="bg-white rounded-2xl border border-slate-200/70 shadow-sm p-8 space-y-4"
                  >
                    ...
                    {[
                      {
                        ...
                        desc: "Receive updates about programs and events",
                      },
                      { label: "SMS Alerts", desc: "Get text messages for urgent updates" },
                      {
                        label: "Monthly Newsletter",
                        desc: "Impact reports and program highlights",
                      },
