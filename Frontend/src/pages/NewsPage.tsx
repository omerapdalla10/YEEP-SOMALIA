...
export default function NewsPage() {
  ...
  const { data: articles, loading, error, refetch } = useCollection<Article>('/news', { limit: 100 })
    ...
    <div className="pt-16 lg:pt-20">
      ...
      <section className="py-20 bg-gradient-to-br from-[#0f766e] to-[#115e59]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <span className="inline-block px-3 py-1 bg-white/20 text-white text-xs font-semibold rounded-full mb-4">News & Blog</span>
          <h1 className="text-4xl lg:text-5xl font-bold text-white mb-5">Latest News & Stories</h1>
          <p className="text-xl text-white/80 max-w-2xl mx-auto">
            Stories of impact, program updates, partnership announcements, and the voices of our community.
          </p>
        </div>
      </section>
              ...
              {categories.map((c) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                    activeCategory === c ? 'bg-[#0f766e] text-white' : 'bg-gray-100 text-gray-600 hover:bg-teal-50 hover:text-[#0f766e]'
                  }`}
                >
                  {c}
                </button>
          ...
          >
            ...
            {featured && (
              <div className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all mb-10 flex flex-col lg:flex-row">
                <div className="relative lg:w-1/2 h-64 lg:h-auto overflow-hidden bg-gray-100">
                  ...
                  />
                  <span className="absolute top-4 left-4 px-3 py-1 bg-[#f59e0b] text-white text-xs font-bold rounded-full">Featured</span>
                </div>
                <div className="p-8 lg:w-1/2 flex flex-col justify-center">
                  ...
                  <div className="flex items-center gap-4 text-xs text-gray-400 mb-5">
                    ...
                    <span className="flex items-center gap-1"><Tag size={11} /> {featured.readTime} read</span>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
              {filtered.map((article) => (
                <div key={article._id} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 flex flex-col">
                  <div className="relative h-44 overflow-hidden bg-gray-100">
                    <img
                      ...
                      className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                    />
                    <span className={`absolute top-3 left-3 px-2.5 py-1 text-xs font-semibold rounded-full ${catColors[article.category]}`}>
                      {article.category}
                    </span>
                  </div>
                  <div className="p-5 flex-1 flex flex-col">
                    <h3 className="font-bold text-gray-900 mb-2 line-clamp-2">{article.title}</h3>
                    <p className="text-xs text-gray-500 leading-relaxed mb-4 flex-1 line-clamp-3">{article.excerpt}</p>
                    <div className="flex items-center gap-3 text-xs text-gray-400 mb-3">
                      <span className="flex items-center gap-1"><Calendar size={10} /> {formatDate(article.publishedAt)}</span>
                      <span className="flex items-center gap-1"><Tag size={10} /> {article.readTime}</span>
                    </div>
              ...
              {[1, 2, 3, '...', 8].map((p, i) => (
                <button
                  ...
                  className={`w-9 h-9 rounded-xl text-sm font-medium transition-colors ${
                    p === 1 ? 'bg-[#0f766e] text-white' : 'bg-white text-gray-500 hover:bg-teal-50 hover:text-[#0f766e] border border-gray-200'
                  }`}
                >
                  {p}
                </button>
