...
export function useCollection<T = unknown>(
  ...
  const [state, setState] = useState<QueryState<T[]>>({ data: [], loading: path !== null, error: null })
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch(
        (err: ApiError) =>
          alive && setState({ data: [], loading: false, error: err.message }),
      )
    return () => {
      alive = false
    }
...
export function useResource<T = unknown>(path: string | null) {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch(
        (err: ApiError) =>
          alive && setState({ data: null, loading: false, error: err.message }),
      )
    return () => {
      alive = false
    }
