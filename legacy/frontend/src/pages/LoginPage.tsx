...
import { FormAlert } from '../components/DataStates'
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(from ?? (user.role !== 'volunteer' ? '/admin' : '/dashboard'), { replace: true })
    } catch (err) {
        ...
        <div className="absolute inset-0 bg-[#0f766e]/85 flex flex-col items-center justify-center p-12">
          <Link to="/" className="flex items-center gap-3 mb-10">
            ...
            <div>
              ...
              <div className="text-white/70 text-xs tracking-wide">Engage · Empower · Transform</div>
            </div>
          </Link>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-6 py-12 bg-white">
        <div className="w-full max-w-sm">
          ...
          <p className="text-gray-400 text-sm mb-8">
            ...
            <Link to="/register" className="text-[#0f766e] font-semibold hover:underline">Create one</Link>
          </p>
          ...
          <div className="mt-6 text-center">
            <Link to="/admin" className="text-xs text-gray-400 hover:text-[#0f766e] transition-colors underline">
              Admin Login →
            </Link>
          </div>
        </div>
