...
export function humanizeIssue(issue: ZodIssue): string {
  ...
  switch (issue.code) {
    ...
    case 'too_small': {
      const n = issue.minimum as number
      if (issue.type === 'string')
        return n <= 1 ? `${field} is required.` : `${field} must be at least ${n} characters.`
      if (issue.type === 'array') return `${field} must have at least ${n} item${n === 1 ? '' : 's'}.`
      return `${field} must be at least ${n}.`
    }
    case 'too_big': {
      const n = issue.maximum as number
      if (issue.type === 'string') return `${field} must be at most ${n} characters.`
      if (issue.type === 'array') return `${field} must have at most ${n} item${n === 1 ? '' : 's'}.`
...
export function formatZodError(err: ZodError): FormattedValidationError {
  ...
  const message =
    keys.length === 1
      ? fields[keys[0]]
      : `Please fix ${keys.length} fields and try again.`
