...
import {
  ArrowRight, Heart, Users, BookOpen, Globe, Star, ChevronRight,
  Play, CheckCircle, Quote, Calendar, MapPin, TrendingUp, Loader2
} from 'lucide-react'
import { useCollection, useResource } from '../lib/hooks'
import { img } from '../lib/img'
import { formatCountPlus, formatMoneyCompact } from '../lib/format'
...
export default function HomePage() {
  ...
  const statCards = [
    { value: stats ? formatCountPlus(stats.youthEmpowered) : '—', label: 'Youth Empowered', icon: Users, color: 'bg-teal-50 text-[#0f766e]' },
    { value: stats ? String(stats.activePrograms) : '—', label: 'Active Programs', icon: BookOpen, color: 'bg-green-50 text-[#16a34a]' },
    { value: stats ? String(stats.communitiesReached) : '—', label: 'Communities Reached', icon: Globe, color: 'bg-amber-50 text-[#f59e0b]' },
    { value: stats ? formatMoneyCompact(stats.fundsRaised) : '—', label: 'Funds Raised', icon: TrendingUp, color: 'bg-teal-50 text-[#0f766e]' },
  ]
    ...
    <div className="pt-16 lg:pt-20">
      ...
      <section className="py-14 bg-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {statCards.map((stat) => (
              <div key={stat.label} className="text-center p-6 rounded-2xl bg-gray-50 hover:shadow-md transition-shadow">
      ...
      <section className="py-20 bg-[#f8fafc]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-12">
            <div>
              <span className="text-[#0f766e] text-sm font-semibold uppercase tracking-wider">What We Do</span>
              <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mt-2">Featured Programs</h2>
            </div>
            <Link to="/programs" className="flex items-center gap-1.5 text-[#0f766e] font-semibold text-sm hover:gap-2.5 transition-all mt-4 sm:mt-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-7">
            {programs.map((prog) => (
              <div key={prog._id} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                ...
                <div className="p-6">
                  ...
                  <p className="text-sm text-gray-500 leading-relaxed mb-4">{prog.summary || prog.description}</p>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-gray-400 flex items-center gap-1">
                      <Users size={12} />
                      {(prog.beneficiaries ?? 0).toLocaleString()} beneficiaries
                    </span>
                    <Link to="/programs" className="text-sm font-semibold text-[#0f766e] hover:text-[#0d9488] flex items-center gap-1 transition-colors">
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
            <div>
              <span className="text-[#0f766e] text-sm font-semibold uppercase tracking-wider">Our Impact</span>
              <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mt-2 mb-5">
                Young People at the<br />Centre of Peace
              </h2>
              <p className="text-gray-500 leading-relaxed mb-7">
                Our youth-led approach tackles the drivers of conflict and exclusion — combining leadership training, community engagement, and policy advocacy so young people help build lasting peace.
              </p>
              ...
              <Link to="/about" className="mt-8 inline-flex items-center gap-2 px-6 py-3 bg-[#0f766e] hover:bg-[#0d9488] text-white font-semibold rounded-xl transition-all shadow-md hover:shadow-lg">
      ...
      <section className="py-20 bg-[#f8fafc]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-12">
            <div>
              <span className="text-[#0f766e] text-sm font-semibold uppercase tracking-wider">Calendar</span>
              <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mt-2">Upcoming Events</h2>
            </div>
            <Link to="/events" className="flex items-center gap-1.5 text-[#0f766e] font-semibold text-sm hover:gap-2.5 transition-all mt-4 sm:mt-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {events.map((ev) => (
              <div key={ev._id} className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow border border-gray-100">
                <span className="text-xs font-semibold text-[#f59e0b] bg-amber-50 px-2.5 py-1 rounded-full">{ev.type}</span>
                ...
                <Link to="/events" className="mt-4 w-full flex items-center justify-center gap-2 py-2.5 border border-[#0f766e] text-[#0f766e] text-sm font-semibold rounded-xl hover:bg-teal-50 transition-colors">
                  Register Now
                </Link>
              </div>
            ))}
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <span className="text-[#0f766e] text-sm font-semibold uppercase tracking-wider">Testimonials</span>
            <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mt-2">Success Stories</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-7">
            {stories.map((story) => (
              <div key={story._id} className="bg-[#f8fafc] rounded-2xl p-6 hover:shadow-md transition-shadow">
      ...
      <section className="py-14 bg-[#f8fafc] border-y border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-center text-sm text-gray-400 font-medium mb-8 uppercase tracking-wider">Trusted by Leading Organizations</p>
          <div className="flex flex-wrap items-center justify-center gap-8">
            {partners.map((p) => (
              <div key={p._id} className="px-6 py-3 bg-white rounded-xl shadow-sm text-gray-400 font-bold text-sm hover:text-[#0f766e] hover:shadow-md transition-all">
                {p.name}
              </div>
            ))}
          </div>
      ...
      <section className="py-20 bg-gradient-to-br from-[#0f766e] to-[#0d9488]">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl lg:text-4xl font-bold text-white mb-4">Stay Connected with YEEP Somalia</h2>
          <p className="text-white/80 mb-8">Get the latest news, program updates, and impact stories delivered to your inbox.</p>
          ...
          <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/volunteer" className="px-7 py-3 bg-[#f59e0b] hover:bg-amber-500 text-white font-semibold rounded-xl transition-all shadow-lg">
              Volunteer Today
            </Link>
            <Link to="/contact" className="px-7 py-3 bg-white/10 hover:bg-white/20 text-white font-semibold rounded-xl transition-all border border-white/30">
              Contact Us
            </Link>
          </div>
        </div>
