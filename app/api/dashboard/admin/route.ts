...
export const GET = route(async (req: NextRequest) => {
  ...
  const [
    ...
    User.aggregate<{ _id: { y: number; m: number }; value: number }>([
      {
        $group: {
  ...
  let running = 0;
  const userGrowth = userGrowthAgg.map((r) => {
    running += r.value;
    return { month: MONTHS[(r._id.m - 1) % 12], users: running };
  });

  const totalPrograms = programByCategory.reduce((s, r) => s + r.value, 0) || 1;
  const programDist = programByCategory.map((r, i) => ({
  ...
  if (recentApplication)
    activity.push({
      ...
      time: relativeTime((recentApplication as unknown as { createdAt: Date }).createdAt),
      type: "volunteer",
    });
  if (recentEvent)
    activity.push({
      text: `${recentEvent.title} — ${recentEvent.registered}/${recentEvent.capacity} registered`,
      time: relativeTime((recentEvent as unknown as { createdAt: Date }).createdAt),
      type: "event",
    });
  if (recentArticle)
    activity.push({
      ...
      time: relativeTime(
        (recentArticle.publishedAt as Date | undefined) ??
          (recentArticle as unknown as { createdAt: Date }).createdAt,
      ),
      type: "news",
    });
  if (recentProject)
    activity.push({
      ...
      time: relativeTime((recentProject as unknown as { updatedAt: Date }).updatedAt),
      type: "project",
    });
