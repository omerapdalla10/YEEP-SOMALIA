...
const benefits = [
  ...
  { icon: Clock, title: 'Flexible Commitment', desc: 'Choose from one-time events, part-time, or full-time volunteering options.' },
]

export default function VolunteerPage() {
  const { data: opportunities, loading, error, refetch } = useCollection<VolunteerRole>('/volunteer-roles', { limit: 100 })
  const { data: stories } = useCollection<Testimonial>('/testimonials', { placement: 'volunteer' })

  const [form, setForm] = useState({
    name: '', email: '', phone: '', role: '', availability: '', motivation: '',
  })
    ...
    <div className="pt-16 lg:pt-20">
      ...
      <section className="relative py-24 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <span className="inline-block px-3 py-1 bg-white/20 text-white text-xs font-semibold rounded-full mb-4">Volunteer</span>
          <h1 className="text-4xl lg:text-5xl font-bold text-white mb-5">Be the Change</h1>
          <p className="text-xl text-white/80 max-w-2xl mx-auto mb-8">
            Join a growing network of young Somali volunteers giving their time and skills to peacebuilding and youth leadership.
          </p>
          <div className="flex justify-center gap-6">
            {[{ label: 'Youth-led', sub: 'Approach' }, { label: '10+', sub: 'Partners' }, { label: 'YPS', sub: 'Our Focus' }].map((s) => (
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <span className="text-[#0f766e] text-sm font-semibold uppercase tracking-wider">Why Volunteer</span>
            <h2 className="text-3xl font-bold text-gray-900 mt-2">What You Gain by Giving</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {benefits.map((b) => (
              <div key={b.title} className="group text-center p-7 rounded-2xl border border-gray-100 hover:border-[#0f766e] hover:bg-teal-50 transition-all">
                <div className="w-14 h-14 rounded-2xl bg-teal-50 group-hover:bg-[#0f766e] flex items-center justify-center mx-auto mb-4 transition-colors">
                  <b.icon size={24} className="text-[#0f766e] group-hover:text-white transition-colors" />
                </div>
      ...
      <section className="py-20 bg-[#f8fafc]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <span className="text-[#0f766e] text-sm font-semibold uppercase tracking-wider">Opportunities</span>
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {opportunities.map((opp) => (
                <div key={opp._id} className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow border border-gray-100">
                  ...
                  <div className="space-y-1.5 text-sm text-gray-500 mb-4">
                    <div className="flex items-center gap-2"><Clock size={13} className="text-[#0f766e]" /> {opp.commitment}</div>
                    <div className="flex items-center gap-2"><Globe size={13} className="text-[#0f766e]" /> {opp.location}</div>
                    <div className="flex items-center gap-2"><CheckCircle size={13} className="text-[#0f766e]" /> {opp.skills}</div>
                  </div>
                  <button
                    onClick={() => {
      ...
      <section id="volunteer-form" className="py-20 bg-white">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            <span className="text-[#0f766e] text-sm font-semibold uppercase tracking-wider">Apply</span>
          ...
          {submitted ? (
            <div className="text-center py-16">
              ...
              <p className="text-gray-500">Thank you for your interest in volunteering with YEEP Somalia. Our team will contact you within 3 business days.</p>
            </div>
          ) : (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  >
                    ...
                    {opportunities.map(o => <option key={o._id} value={o.role}>{o.role}</option>)}
                  </select>
                </div>
              </div>
              <div>
      ...
      <section className="py-16 bg-[#f8fafc]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-7">
            {stories.map((s) => (
              <div key={s._id} className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="ml-auto flex">
                    {Array.from({ length: 5 }).map((_, i) => <Star key={i} size={12} className="fill-[#f59e0b] text-[#f59e0b]" />)}
                  </div>
                </div>
              </div>
            ))}
